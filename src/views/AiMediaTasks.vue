    ...
    for (const task of tasksToProcess) {
        ...
        try {
            ...
            const generateRes = await fetch('/api/generate-image', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ prompt: task.prompt, apiKey: apiKey.value })
            });
            if (!generateRes.ok) {
                const errData = await generateRes.json().catch(() => ({}));
                throw new Error(errData.error || `HTTP error ${generateRes.status}`);
            }
            const { base64Data } = await generateRes.json();
