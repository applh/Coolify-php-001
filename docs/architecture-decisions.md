...
2. **Yaw/Pitch Touch-Controlled Camera Orbits**
   - **Choice**: Coupled Compose pointer drag events to floating Yaw and Pitch state parameters, allowing users to physically "swivel" or orbit around the dungeon grid or casino card table felt in real-time.
   - **Reason**: Enhances gaming feedback. Under touch-driven orbits, players can rotate around obstacles or examine cards from custom angles, bringing real-world depth and tactile agency to standard flat mobile screens.
