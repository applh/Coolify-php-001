...
The `taxonomy.json` file maps the first two levels: the 10 Parts and 100 Modules.
- **Benefit**: Zero-latency concept lookups for the high-level roadmap.

### 3.2 Layer 2: The Part Manifests (JSON)
Each Part (1-10) has a dedicated `Part-X-Manifest.json` (or handled directly in AI Studio folders). These manifests contain the 100 Hours (1 hour each) and their slide generation metadata.
- **Benefit**: AI Studio agents can "skim" all the hours in a part within a single context window to improve or expand them.

### 3.3 Layer 3: The Deep Dive / Slide Generation (Markdown / UI)
Once an agent or user identifies a specific 1-hour Hour, it loads a dedicated Markdown file (or a specific block in a "Big Markdown" file) for the full code example and explanation. The deployed app takes this logic to generate the 60 slides.
- **Benefit**: Preserves syntax highlighting and formatted technical instructions for the user.

## 4. Storage Limits in AI Studio
- **File Explorer**: Avoid lists > 50 files in a single folder. Use sub-directories per module or part.
